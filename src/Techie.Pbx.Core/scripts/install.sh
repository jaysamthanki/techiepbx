...
# The privileged helper (D142). Its own directory, outside /opt/tnpbx: the web user owns that
# tree, and a root binary the web user could rewrite would be a way to become root. The state
# directory holds the last applied nftables ruleset, which the helper re-applies at boot.
...
apply_layout() {
  ...
  # The privileged helper, and the strictest thing on this box (D142). Its binary directory is
  # root:root 0755 — world-readable costs nothing and there is no secret in it, but nothing
  # except root may write it, because the web user being able to rewrite a root binary would
  # undo the whole reason the helper exists instead of a sudo rule. Empty until app-deploy.sh
  # puts the published helper in it.
  run chown root:root "$HELPER_HOME"
  run chmod 0755 "$HELPER_HOME"

  # Its state: the last applied nftables ruleset and the record of what was in it. 0700 root,
  # because nothing but the helper has any business reading or writing what this box's packet
  # filter will be at the next boot.
  run chown root:root "$HELPER_STATE_DIR"
  run chmod 0700 "$HELPER_STATE_DIR"
}
apply_layout
...
# --- 7b. the privileged helper -------------------------------------------------
# The one root service (D142). Its unit and the tmpfiles entry for its socket directory live in
# the repo beside this script, so there is exactly one copy of each and app-deploy.sh installs
# the same files from the publish tarball rather than a second version of them.
#
# nftables is what it drives, so the package has to be here. Debian has it in the base install,
# but a minimal cloud image may not.

log "installing nftables (the helper's firewall backend)"
run apt-get install -y -q --no-install-recommends nftables

# The firewall this box runs is generated by the application and applied by the helper, so
# nftables' own /etc/nftables.conf is not ours and is not enabled here. The helper's table is
# separate from fail2ban's (D143), and each replaces only its own.
log "nftables.service is left as the distribution has it; the helper manages its own table"

if [[ -f "${SCRIPT_DIR}/tnpbx-helper.tmpfiles.conf" ]]; then
  log "installing /etc/tmpfiles.d/tnpbx-helper.conf (the socket directory, 0750 root:${APP_USER})"
  run mkdir -p /etc/tmpfiles.d
  run install -o root -g root -m 0644 "${SCRIPT_DIR}/tnpbx-helper.tmpfiles.conf" /etc/tmpfiles.d/tnpbx-helper.conf
  run systemd-tmpfiles --create /etc/tmpfiles.d/tnpbx-helper.conf
else
  warn "tnpbx-helper.tmpfiles.conf is not beside this script; /run/tnpbx will only exist while the helper runs"
fi

if [[ -f "${SCRIPT_DIR}/tnpbx-helper.service" ]]; then
  log "installing /etc/systemd/system/tnpbx-helper.service"
  run install -o root -g root -m 0644 "${SCRIPT_DIR}/tnpbx-helper.service" /etc/systemd/system/tnpbx-helper.service
  run systemctl daemon-reload
  run systemctl enable tnpbx-helper

  # Enabled but NOT started, for the same reason asterisk.service is: there is nothing to run
  # yet. ${HELPER_HOME} is empty until app-deploy.sh unpacks the published helper into it, and
  # that script starts the service.
  log "tnpbx-helper.service is enabled but NOT started (${HELPER_HOME} is empty until the app deploy)"
else
  warn "tnpbx-helper.service is not beside this script; the firewall will not be managed until it is installed"
fi
...
Created:
  ...
  ${HELPER_HOME}        root:root  0755  the privileged helper's binary
                                            (empty until the app deploy)
  ...
  tnpbx-helper.service     the one root service, enabled but not yet started
  /etc/tmpfiles.d/tnpbx-helper.conf
                           /run/tnpbx  root:${APP_USER}  0750  (the helper socket)
...
Deliberately NOT done:
  ...
  * The Helper is installed and enabled but NOT running: its binary arrives
    with app-deploy.sh, which starts it (D142).
  * No firewall rules are loaded yet. The helper applies them when an admin
    presses Apply on Settings -> Firewall, and re-applies them at every boot
    after that. Until then this box has no packet filter of its own (D143).
...
Next steps:
  ...
  5. Settings -> Firewall: check the helper says "Reachable", then Apply. That
     loads the ruleset — SIP, RTP and the admin UI, plus loopback, established
     connections, ICMP and SSH, which the helper always adds and no message can
     remove (D142, D143). Until that is done, restrict UDP 5060 and 10000-20000
     to known addresses at the cloud firewall: an open 5060 is found by scanners
     in minutes.
