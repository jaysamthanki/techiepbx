    ...
    public static class PjsipConfRenderer
    {
            ...
            string confDirectory)
        {
            ...
            foreach (var extension in ConfText.EnabledInOrder(extensions))
            {
                ...
                // How many devices may be registered at once, one setting for every extension
                // (D110). With more than one, remove_existing must be off: it deletes every
                // other contact on a new REGISTER, which is the opposite of an office phone
                // plus a laptop softphone coexisting. Stale contacts still expire on their own.
                sb.Append($"max_contacts = {transport.MaxContacts}\n");

                if (transport.MaxContacts > 1)
