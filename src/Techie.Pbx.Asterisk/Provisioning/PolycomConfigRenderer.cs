...
namespace Techie.Pbx.Asterisk.Provisioning
{
    ...
    public static class PolycomConfigRenderer
    {
        ...
        public static string Render(PolycomConfig config)
        {
            ...
            // whether or not anyone has told it which extension it is. The offset is the zone's
            // STANDARD one (D150): the phone adds its own daylight-saving hour on top whatever
            // the config says — D149's attempt to switch that rule off did not hold on a real
            // Edge E450 — so the number must not already contain the summer hour or it is
            // counted twice and the phone runs fast (D149's one-hour-fast symptom). The two
            // overrideDHCP flags pin both values over anything the site's DHCP server may be
            // offering for the time (D149).
