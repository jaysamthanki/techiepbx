...
namespace Techie.Pbx.Asterisk.Provisioning
{
    ...
    /// gets an answer to (D77, D151, D153):
    ///
    /// <code>
    ...
    /// GET /polycom/logo.png                the site's logo, when it is a PNG
    /// GET /polycom/logo.jpg                the same, when it is a JPEG
    /// </code>
    ///
    ...
    /// not twelve lowercase hex digits gets that far. The image names are four literals.
    /// </summary>
    public static partial class PolycomFiles
    {
        ...
        /// <summary>The logo's name without its extension, which the format supplies.</summary>
        private const string LogoStem = "logo";
        ...
        /// <summary>
        /// The name the logo is served under, and so the last segment of the URL every phone's
        /// config names in <c>bg.logo</c>. It ends in the real extension, as the background's does.
        /// </summary>
        public static string LogoFileName(BackgroundImageFormat format) =>
            LogoStem + BackgroundImageSignature.Extension(format);
        ...
        public static bool TryParseBackground(string? fileName, out BackgroundImageFormat format) =>
            TryParseImage(BackgroundStem, fileName, out format);
        ...
        /// <summary>
        /// Whether this is a request for the logo, and in which format. The same exact match as
        /// the background's.
        /// </summary>
        public static bool TryParseLogo(string? fileName, out BackgroundImageFormat format) =>
            TryParseImage(LogoStem, fileName, out format);
        ...
        /// <summary>An exact, ordinal match on the stem plus one of the two extensions.</summary>
        private static bool TryParseImage(string stem, string? fileName, out BackgroundImageFormat format)
        {
            foreach (var candidate in new[] { BackgroundImageFormat.Png, BackgroundImageFormat.Jpeg })
            {
                if (string.Equals(fileName, stem + BackgroundImageSignature.Extension(candidate), StringComparison.Ordinal))
                {
                    format = candidate;
                    return true;
                }
