...
{
    /// <summary>
    /// The push this app sends a Polycom phone's own web UI to reload its config (D86). Rebooting
    /// left this route for a SIP NOTIFY, which works through NAT (D123), so a config reload is all
    /// that is pushed now. The network call is replaced with a fake handler so these run without a
    /// real phone: production leaves <c>handler</c> null and gets a real one instead.
    /// </summary>
