...
| 41 | Polycom logo, and exact-size validation for both phone images | supporting | **Built 2026-09-24, lab-verified 2026-09-24** (D153) — strict size enforced live through the page's upload handler (640x480 rejected with the exact message, 320x240 accepted and served), `bg.logo` rendered when a logo file exists and absent when it does not, `/polycom/logo.png` serving 200 behind the UA+auth gate. Part 1, no UI: `LogoStore` beside `BackgroundStore` (both now thin subclasses of `PolycomImageStore`), stored as `Data/polycom-logo.{png,jpg}`, served as `/polycom/logo.{png,jpg}` behind the same gate, rendered as `bg.logo` in the existing `<bg>` element (golden files `polycom-phone-logo.cfg`, `polycom-phone-background-logo.cfg`; all existing ones unchanged). Uploads must be exactly 320x240/800x480 (background) or 60x26/182x78 (logo), read from the PNG IHDR or the JPEG start-of-frame with no image package. **Next:** logo upload/remove UI on the Phones page. **Not verified:** that a Poly Edge honours `bg.logo`, and the image fetch answering the 401 (D151). |
