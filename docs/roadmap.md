...
| 38 | Polycom on-call soft keys: Park + Blind Xfer | supporting | **Built 2026-09-23; desk test found Park works and Blind Xfer ($Fblindxfer$) did nothing on a real Edge E450 — fixed 2026-09-23 (D144 amended + D147); slot-key retrieval fixed same day — parking keys are attendant type normal, automata made a lit slot attempt a pickup that does nothing (D148): Blind Xfer is an EFK prompt + `$Trefer$`; the real reason no transfer worked was `res_pjsip_refer` missing from the module allowlist, now added (D144). A fixed pair appended to every Polycom's active-call soft key set: Blind Xfer via `$Fblindxfer$`, Park as an EFK sending DTMF `*3` (the configurable park DTMF code) mid-call — written only when parking is enabled. |
| 39 | Polycom background logo: upload an image, set it on every provisioned Polycom | supporting | **Built 2026-09-23** (D145, D151, D152); desk test pending. One site-wide PNG/JPEG, magic-byte validated, capped at 2 MB, stored as `Data/polycom-background.{png,jpg}` (no schema change, no settings key) and served as `/polycom/background.{png,jpg}` behind the existing basic-auth + Polycom User-Agent gate. The renderer writes a `<bg>` element — `bg.background.enabled`, `bg.color.selection="2,1"`, `bg.color.bm.1.name` — only when an image exists, so with no image the config is byte-identical. UI: one status line + upload/remove modal at the top of the Phones page. **Not verified:** that a Poly answers the 401 on the image fetch with its provisioning credentials (D151). |
| 40 | Line display name "Ext - Name" on every provisioned phone | supporting | **Spec'd** (D146). Polycom `reg.1.displayName` and the Yealink equivalent render as `<ext> - <name>`; empty name shows the number alone. Small renderer change; queued behind the piece 38 Blind Xfer fix (same files). |
| 41 | Polycom logo, and auto-resized phone images | supporting | **Built, lab-verified, desk-verified** — amended 2026-09-24 by **D154**: uploads of any size are now resized instead of refused (background to 320x240, cover + centre-crop; logo to 60x26, contain + transparent padding; output PNG), an upload already exactly the target size is stored byte for byte in its own format, 2 MB cap still checked before decoding, plus a 25-megapixel cap on the declared size. First image dependency: SixLabors.ImageSharp 3.1.12 in `Techie.Pbx.Asterisk` (`PolycomImageResizer`, PNG + JPEG codecs only). The D153 part was lab-verified 2026-09-24 (strict size enforced live, `bg.logo` rendered when a logo exists and absent when not, `/polycom/logo.png` serving 200 behind the UA+auth gate) and is unchanged apart from the size rule: `LogoStore` beside `BackgroundStore` (thin subclasses of `PolycomImageStore`), stored as `Data/polycom-logo.{png,jpg}`, served as `/polycom/logo.{png,jpg}`, rendered as `bg.logo` in the existing `<bg>` element (golden files `polycom-phone-logo.cfg`, `polycom-phone-background-logo.cfg`; renderer output unchanged by D154). UI: status lines, previews and the "Background image…" / "Logo…" upload-remove modals live in the Settings tab's Polycom pane (moved off the page top 2026-09-24). **D155, 2026-09-24, desk-verified:** a real Edge E450 on 8.6.1 fetched its config-named logo.png WITHOUT resending the provisioning credentials and never retried the 401 — so the two images are served on the Polycom User-Agent gate alone, and Basic auth stays on the master and per-phone config files (D151's open question answered: the phone does NOT answer the 401). |
