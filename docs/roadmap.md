...
| 38 | Polycom on-call soft keys: Park + Blind Xfer | supporting | **In progress** (D144). A fixed pair appended to every Polycom's active-call soft key set: Blind Xfer via `$Fblindxfer$`, Park as an EFK sending DTMF `*3` (the configurable park DTMF code) mid-call — written only when parking is enabled. |
