...
| 10 | Outbound routes (international restricted by default) | supporting | Done 2026-09-18 (D44–D48). Lab-verified: `us-domestic` route live in dialplan (`outbound-us-domestic` included from `internal` in order, fail-closed `outbound-blocked`), trunk still Registered. International patterns refused by validation; route names must be context-safe (letters/digits/dashes). Outbound audio call pending: dial 17771234567 from a registered softphone. |
