...
| 39 | Polycom background logo: upload an image, set it on every provisioned Polycom | supporting | **Spec'd** (D145). One site-wide PNG/JPG, magic-byte validated, size-capped, stored in the app data dir and served through the existing `/polycom` basic-auth gate (no wwwroot, no anonymous dir). Renderer writes `bg.background.enabled` + `bg.color.bm.1.name` only when an image exists. UI: one upload/remove control on the Phones page. |
