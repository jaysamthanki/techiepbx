...
| 36 | Call flow control (day/night override): toggle from a phone, two destinations (F9) | F9 | **Requested 2026-09-23, next after piece 15.** A CFC entity is a destination: off → the everyday destination (typically the time condition), on → the override destination (the off-hours behaviour without waiting for the clock). Toggled by a press on a provisioned phone key. Open questions resolved by the user 2026-09-23: toggle from any phone holding the assigned key or knowing the feature code; the key lamp shows the state (parking-slot hint pattern); several independent CFCs, chaining only as ordinary destinations. Cross-feature loops stay the admin's problem (D136 amendment). |
