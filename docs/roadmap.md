...
| 23 | Certificate management: in-process ACME (Certes, HTTP-01), Kestrel on 80/443, SIP TLS, auto-renewal | security | **Built 2026-09-22, pending live certificate order** (D97–D101). `013_certificates.sql` + `CertificateRepository` (Current() = newest usable), Certes-based `AcmeCertificateService` answering its own HTTP-01 challenges on port 80 (anonymous `/.well-known/acme-challenge/`), `WebBindings` (80 + 8080 always, 443 with a usable cert), daily renewal at ≤30 days (`CertificateRenewalService`), Certificates page, combined `tnpbx-cert.pem` written by the apply feeding a generated pjsip `transport-tls` on `Sip.TlsPort` (default 5061, restart-set) — D71 finally real. app-deploy.sh unit updated: no `ASPNETCORE_URLS` (app picks bindings) + `CAP_NET_BIND_SERVICE`. 857 tests. Live order for pbx.example.com pending Azure NSG. |
