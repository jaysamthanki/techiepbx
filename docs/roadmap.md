...
| 41 | Polycom logo, and auto-resized phone images | supporting | **Built, pending lab verification** — amended 2026-09-24 by **D154**: uploads of any size are now resized instead of refused (background to 320x240, cover + centre-crop; logo to 60x26, contain + transparent padding; output PNG), an upload already exactly the target size is stored byte for byte in its own format, 2 MB cap still checked before decoding, plus a 25-megapixel cap on the declared size. First image dependency: SixLabors.ImageSharp 3.1.12 in `Techie.Pbx.Asterisk` (`PolycomImageResizer`, PNG + JPEG codecs only). The D153 part was lab-verified 2026-09-24 (strict size enforced live, `bg.logo` rendered when a logo exists and absent when not, `/polycom/logo.png` serving 200 behind the UA+auth gate) and is unchanged apart from the size rule: `LogoStore` beside `BackgroundStore` (thin subclasses of `PolycomImageStore`), stored as `Data/polycom-logo.{png,jpg}`, served as `/polycom/logo.{png,jpg}`, rendered as `bg.logo` in the existing `<bg>` element (golden files `polycom-phone-logo.cfg`, `polycom-phone-background-logo.cfg`; renderer output unchanged by D154). UI: a second status line + "Logo…" upload/remove modal under the background's at the top of the Phones page, built the same way (`phoneLogoChanged` refreshes it). **Not verified:** that a Poly Edge honours `bg.logo`, and the image fetch answering the 401 (D151). |
