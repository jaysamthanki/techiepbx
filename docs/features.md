...
| **Music on hold** | Call parking | **Built 2026-09-19** (D119). One class, `mode=files` on `/var/lib/asterisk/moh`, tracks uploaded and converted like announcements (D55). No per-class UI. The only thing that plays it today is a parked call; ring groups and transfers do not ask for it. |
| **Call parking** | — | **Built 2026-09-19** (D119). One lot, off by default. `*3` (configurable) parks the call you are on, the system speaks the slot back, and dialling the slot number from any phone picks it up. 1–9 slots, 30–600s timeout, then it rings back whoever parked it. Parked callers hear silence or the music on hold above. Own page at `/Parking`. A slot can be put on a phone key, and its lamp is lit while a call is sitting there (D121). Not built: per-user or per-department lots, a park-and-page button. |
| **Phone provisioning** | Extensions | **Built 2026-09-21** (D77–D83). Polycom only. DHCP option 160 points phones at `http://user:pass@host/polycom`; a phone fetches `<mac>.cfg` then `exten<mac>.cfg`, both generated from the database per request and never stored on disk (D79). A phone with valid credentials and a Polycom User-Agent **adds itself** the first time it asks (D78); an admin then names it and gives it an extension. Basic auth against two settings keys, outside the Entra cookie (D77). Eight assignable keys per phone — an extension (BLF + quick dial) or a parking slot — rendered for Polycom only (D121). Not built: firmware serving (D83), log/overrides/contacts upload endpoints (D79), softkeys, more than one line, Yealink keys. |
