...
| **Phone provisioning** | Extensions | **Built 2026-09-21** (D77–D83). Polycom only. DHCP option 160 points phones at `http://user:pass@host/polycom`; a phone fetches `<mac>.cfg` then `exten<mac>.cfg`, both generated from the database per request and never stored on disk (D79). A phone with valid credentials and a Polycom User-Agent **adds itself** the first time it asks (D78); an admin then names it and gives it an extension. Basic auth against two settings keys, outside the Entra cookie (D77). Eight assignable keys per phone: **key 1 is the line it registers as** and the rest are lamps — another extension (BLF + quick dial) or a parking slot (D121, schema 020). Rebooting a phone is a SIP NOTIFY to its registered contact, so it works through NAT (D123). **Yealink** (D88–D90) uses DHCP option 66 and `/yealink` on the same gate, and gets the same web UI passwords from the same two settings (`security.user_password`, D133), the same Reboot button (`check-sync;reboot=true`, D134) and dial-now rules for the eager-safe numbers only — N11, ten and eleven digits (D135). Not built: firmware serving (D83), log/overrides/contacts upload endpoints (D79); built since: on-call soft keys, Park + Blind Xfer (D144). **Phone background/logo: built 2026-09-23** (piece 39, D145, D151, D152) — one site-wide PNG/JPEG (magic-byte validated, 2 MB cap) uploaded on the Phones page, kept in the data folder, served through the `/polycom` basic-auth gate and set on every provisioned Polycom via `bg.background.enabled` + `bg.color.selection` + `bg.color.bm.1.name`; no image means no `bg` block. Polycom only; the image fetch answering the auth challenge is not desk-verified yet. **Logo + auto-resize: built 2026-09-24, pending lab verification** (piece 41, D153, D154) — a second site-wide PNG/JPEG, the logo, stored and served the same way (`/polycom/logo.{png,jpg}`) and written as `bg.logo`; any size of upload is accepted and resized to Poly's E400-series size (background 320x240, scaled to cover and centre-cropped; logo 60x26, scaled to fit and padded with transparency) and stored as a PNG, while an upload already exactly that size is stored byte for byte in its own format. Both are uploaded and removed from the top of the Phones page, each with its own status line and modal. Still open: for Yealink, a per-phone local SIP port and missed-call tracking (parameters not verified); for Polycom, the SNTP `tcpIpApp.sntp.address.overrideDHCP` / `tcpIpApp.sntp.gmtOffset.overrideDHCP` flags (user note 2026-09-22: a site's DHCP must not override the provisioned NTP server and offset). |
